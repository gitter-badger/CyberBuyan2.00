...
function testb(){
    ...
    describe("dht test", function() {
      ...
      it("address comparison should return some distance",function(){
        //simple substraction
        var a = dht.compareAddressesDiff("0","1");
        expect(a).toBe(1);
      });
      it("address padding comparison should return some distance",function(){
        //simple substraction
        a = dht.compareAddressesDiff("0","1000");
        expect(a).toBe(6328);
      });
      it("address padding reverse comparison should return some distance",function(){
        //padding check the other way around
        a = dht.compareAddressesDiff("1000","1");
        expect(a).toBe(-5328);
      });
      it("should tell me that peer is upstream", function() {
        var a = dht.compareAddresses("0","1");
        expect(a).toBe(true);
        //sinon.stub(dht,'');
        //var iterfacexd_dht=i("dht"+id,dht);
        //expect(dht.id).toBe(interfacexd.)
      });
      it("should tell me that peer is downstream", function() {
        a = dht.compareAddresses("1","0");
        expect(a).toBe(false);
      });
      it("should tell me that eq peer is upstream", function() {
        a = dht.compareAddresses("1","1");
        expect(a).toBe(false);
      });
      it("should tell me that eq peer is upstream", function() {
        a = dht.compareAddresses("1","1");
